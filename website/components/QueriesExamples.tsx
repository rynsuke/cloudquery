...
const SecurityQuery = () => (
  <>
    ...
    <div>
      <span> aws_elbv2_load_balancers</span>
    </div>
    <div>
      <span style={{ color: "var(--shiki-token-keyword)" }}>WHERE</span>
    </div>
    <div>
      {" "}
      scheme<span style={{ color: "var(--shiki-token-keyword)" }}> = </span>
...
const ComplianceQuery = () => (
  <>
    ...
    <div>
      <span> require_uppercase_characters</span>
    </div>
    <div>
      <span style={{ color: "var(--shiki-token-keyword)" }}>FROM</span>
    </div>
    <div>
      <span> aws_iam_password_policies</span>
    </div>
    <div>
      <span style={{ color: "var(--shiki-token-keyword)" }}>WHERE</span>
    </div>
...
const QueryAcrossApps = () => (
  <>
    ...
    <div>
      <span> aws_iam_users</span>
    </div>
    <div>
      <span style={{ color: "var(--shiki-token-keyword)" }}> JOIN</span>{" "}
      aws_iam_user_tags{" "}
      <span style={{ color: "var(--shiki-token-keyword)" }}>ON</span>{" "}
      aws_iam_users.id = aws_iam_user_tags.user_id
    </div>
    <div>
      <span style={{ color: "var(--shiki-token-keyword)" }}> JOIN</span>{" "}
      okta_users <span style={{ color: "var(--shiki-token-keyword)" }}>ON</span>{" "}
      aws_iam_users.tags.value = okta_users.profile_email
    </div>
    <div>
      <span style={{ color: "var(--shiki-token-keyword)" }}>WHERE</span>
    </div>
...
const QueryItem = ({ children, onClick, description, title }) => {
  const codeClasses =
    "w-fit h-full px-4 py-3 font-mono text-sm font-medium text-gray-600 bg-black bg-opacity-5 dark:bg-white dark:text-gray-300 dark:bg-opacity-5 betterhover:hover:bg-gray-50 betterhover:dark:hover:bg-gray-900 md:py-3 md:text-base md:leading-6 md:px-10";
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="mt-6">
        <div className="text-lg font-medium dark:text-white text-center">
          {title}
        </div>
        <div className="mt-2 text-center text-base font-medium text-gray-500 dark:text-gray-400">
          {description}
        </div>
      </div>
    </div>
